...
OWN-WORLD: cold near-black ground, slightly tinted per sector by the hue of that sector's star (chroma under .04: steel, teal, sage, umber, ash-lilac, rose). Gold = KRIEFNE, home and its instrument (never an enemy). Oxide red = harm: every round, blast, beam, hazard and tell, and any hostile in the moment it commits. Pigment = who a hostile is: one muted OKLCH colour per servitor and per god (`PIGMENT_DEF`), learned across runs the way the old hulls were. Hue outside 15–118 (gold's and red's bands), lightness 0.58–0.74, chroma ≤ 0.12, ≥ ΔE 0.13 from gold, red and hydrogen, ≥ 0.09 between any two gods, any two servitors, and each god and its own chaff (enforced in test.js). Enraged gods show it by line density (a ticked outer ring) in their own pigment, not by turning red. Wreckage lit edges stay silver. Unmet gods show no pigment anywhere. Amended 2026-09-12 at the user's request: all-red hostiles read as monotonous. Hydrogen blue-white = salvage only. Bare-metal silver = wreckage and neutral text. Every mark is a monoline engraved stroke; light is expressed by line density and hatching, never glow. Wide monoline engraver's capitals plus a narrow tabular technical face. Rank = rings, rarity = rim ticks, heavy = double rule. The committed choice is the only solid line; the rest are dashed. One light per sector. Codex portraits at a registered scale. Inversion to gold field and black engraving only on the nest's hoard recovery.
