...
const UPGRADES=[
 ...
 {id:'vamp', name:'Vampire Chip', desc:'Siphons the slain into hull; the parasite sips output -1% damage', max:5, dyn(p){ return p.vamp>0?{name:'Vampire Chip',desc:'Feed harder: +1 HP per kill (now '+p.vamp+')'}:null; }, apply(p){ p.vamp=(p.vamp||0)+1; p.dmgMult*=0.99; }},
 {id:'seek', name:'Seeker Rounds', desc:'Guidance fins steer bullets onto foes; heavier flight -2.5% speed', max:2, r:1, apply(p){ p.homing+=1; p.projSpeed*=0.975; }},
 {id:'rico', name:'Ricochet Core', desc:'Hardened shot bounces off walls; each impact hits -2% softer', max:2, r:1, apply(p){ p.bounce+=1; p.dmgMult*=0.98; }},
 {id:'inc', name:'Incendiary Rounds', desc:'Phosphor payload sets foes burning; -1.5% impact damage', max:2, r:1, apply(p){ p.inc+=1; p.dmgMult*=0.985; }},
 ...
 {id:'magnet', name:'Magnet Core', desc:'Vacuum coils widen the pickup; power draw -1% rate', max:3, r:0, dyn(p){ return (p.magnet>90||p.pull>430)?{name:'Magnet Core',desc:'Vacuum EVERY gem + wider pickup'}:null; }, apply(p){ p.magnet*=2.2; p.pull=(p.pull||430)*1.4; p.fireRate*=0.99; collectGems(); }},
 {id:'pcell', name:'Portal Cell', desc:'ITEM: +2 recall charges (max 5)', req(p){ return p.charges<5; },
  dyn(p){ return p.recallUnlocked?{name:'Portal Cell',desc:'Spare charges ride along: +2 gate charges, -0.5% rate'}:{name:'Portal Cell',desc:'UNLOCK recall (E): +2 gate charges'}; }, apply(p){ if(p.recallUnlocked) p.fireRate*=0.995; p.recallUnlocked=true; p.charges=Math.min(5,p.charges+2); }},
 {id:'gatecd', name:'Gate Overdrive', desc:'Hotter gate, shorter wait; -0.5% rate', max:3, r:1, req(p){ return p.recallUnlocked; }, apply(p){ p.recallCdMax=Math.max(2,p.recallCdMax*0.75); p.fireRate*=0.995; }},
 {id:'transit', name:'Instant Transit', desc:'Tuned blink, shorter channel; -0.5% rate', max:3, r:1, req(p){ return p.recallUnlocked; }, apply(p){ p.channelMax=Math.max(0.1,p.channelMax-0.2); p.fireRate*=0.995; }},
 {id:'orbit', name:'Guardian Orbit', desc:'Each blade needs guiding; +1 orb, -1% damage', max:2, r:1, apply(p){ p.orbs+=1; p.dmgMult*=0.99; }},
 ...
 {id:'shock', name:'Kinetic Discharge', desc:'Kill-charge coils vent a shockwave every 50 kills; -1% rate', max:1, r:1,
  apply(p){ p.shockOn=true; p.shockNeed=50; p.shockKills=0; p.shockDmg=28; p.shockR=180; p.fireRate*=0.99; }},
 ...
 // ---------- heavy ordnance ----------
 {id:'orbital', name:'Orbital Cannon', desc:'Strike uplink calls it down; tracking -1% rate', max:3, r:1,
  dyn(p){ return p.orbitalLvl>0?{name:'Orbital Cannon',desc:'Faster cadence, +1 strike per salvo (now '+p.orbitalLvl+')'}:null; },
  apply(p){ p.orbitalLvl++; p.orbitalCd=Math.max(3.5,9-(p.orbitalLvl-1)*2); p.orbitalT=Math.min(p.orbitalT||99,3); p.fireRate*=0.99; }},
 {id:'lance', name:'Prism Lance', desc:'The prism drinks pump power; periodic beam, -1% rate', max:3, r:1,
  dyn(p){ return p.lanceLvl>0?{name:'Prism Lance',desc:'Wider beam, faster cycle (now '+p.lanceLvl+')'}:null; },
  apply(p){ p.lanceLvl++; p.lanceCd=Math.max(2.2,6-(p.lanceLvl-1)*1.4); p.lanceT=Math.min(p.lanceT||99,2); p.fireRate*=0.99; }},
 // ---------- ammunition ----------
 {id:'flak', name:'Flak Rounds', desc:'Shells burst into splash; -2% direct damage', max:2, r:1,
  apply(p){ p.flak+=1; p.dmgMult*=0.98; }},
 {id:'corrode', name:'Corrosive Rounds', desc:'Acid eats armour (+8% taken, stacks 5); volatile mix -1% rate', max:2, r:1,
  apply(p){ p.corrode+=1; p.fireRate*=0.99; }},
 {id:'chain', name:'Chain Shot', desc:'Charge arcs to a nearby foe; split load -1.5% damage', max:2, r:1,
  apply(p){ p.chain+=1; p.dmgMult*=0.985; }},
 {id:'overcharge', name:'Overcharge Rounds', desc:'Every 5th shot leaves a heavy slug; charge time -1% rate', max:2, r:1,
  apply(p){ p.overcharge+=1; p.fireRate*=0.99; }},
 // ---------- sustain and pressure ----------
 {id:'adrenal', name:'Adrenal Core', desc:'Damage rises as the hull falls (to +50%); taxed when calm -1% base rate', max:2, r:1,
  apply(p){ p.adrenal+=1; p.fireRate*=0.99; }},
 {id:'repair', name:'Repair Drone', desc:'Drones knit the hull after 4s calm; bay mass -1% damage', max:3, r:1,
  apply(p){ p.repair+=1.5; p.dmgMult*=0.99; }},
 {id:'shrap', name:'Shrapnel Core', desc:'The slain burst; volatile kills, smaller bullets -1% damage', max:2, r:1,
  apply(p){ p.shrap+=1; p.dmgMult*=0.99; }},
 {id:'salvage', name:'Salvage Protocol', desc:'Gems mend hull and pay +15% XP; sorting takes cycle -1% rate', max:2, r:0,
  apply(p){ p.salvage+=1; p.xpBonus*=1.15; p.fireRate*=0.99; }}
];
...
let codexSummonRects=[]; // clickable god-names on the SUMMONS line (rebuilt per frame)
...
// from ENFORCER up, so the climb reads the way it is flown: S5 first, the Apex
// last. Headers are not selectable.
function codexRows(){
 const L=codexList(), rows=[];
 if(codexTab!=='bosses'){ L.forEach((entry,i)=>rows.push({entry,i})); return rows; }
 for(let t=1;t<TIER_NAMES.length;t++){
...
function handleClick(x,y){
  ...
  if(state==='codex'){
   ...
   for(const r of codexSummonRects){ if(r.id&&inBtn(x,y,r)){
    const k=codexList().findIndex(e=>e.id===r.id);
    if(codexTab==='bosses'&&k>=0){ if(codexSel!==k) codexPage=0; codexSel=k; SFX.click(); return; }
   } }
...
// Who a wounded god summons, read from the ladder itself so the codex can
// never disagree with who actually answers a call. Unmet gods stay ???.
// The detail plate draws each name as a link into its own entry; the string
// below is the same words for the live region.
function commandParts(kind){
 const c={};
 for(const k of summonsOf(kind)) c[k]=(c[k]||0)+1;
 return Object.keys(c).map(k=>({id:k,name:codexSeen(k)?BOSSDEF[k].name:'???',known:codexSeen(k),n:c[k]}));
}
function commandLine(kind){
 const down=commandParts(kind).map(p=>p.name+(p.n>1?' ×'+p.n:''));
 const th=thrallDebut(kind); // its thralls, the lesser copies that walk common sectors (spec §8)
 return 'Summons '+(down.length?down.join(', '):'chaff at half strength')+(th<Infinity?' · Thralls walk the trail from S'+th:'');
}
...
// The SUMMONS line under a god's name: one label, then a link per god it
// calls when wounded. A met god's name is drawn in its own pigment and opens
// that entry on click; an unmet one stays dim and inert. Returns the y below
// the last line drawn.
function drawSummonsLine(entryE,tx,y,maxW){
 ctx.textAlign='left'; ctx.font=fM(11);
 const segs=[{t:'SUMMONS ',col:K.textDim,id:null}];
 const parts=commandParts(entryE.id);
 if(!parts.length) segs.push({t:'CHAFF AT HALF STRENGTH',col:K.textDim,id:null});
 parts.forEach((p,i)=>{
  if(i) segs.push({t:', ',col:K.textDim,id:null});
  const pg=PIG[p.id];
  segs.push({t:p.name+(p.n>1?' ×'+p.n:''),col:(p.known&&pg)?pg.hi:K.textDim,id:p.known?p.id:null});
 });
 let x=tx, yy=y;
 for(const s of segs){
  const w=ctx.measureText(s.t).width;
  if(x+w>tx+maxW&&x>tx){ x=tx; yy+=15; }
  ctx.fillStyle=s.col; ctx.fillText(s.t,x,yy);
  if(s.id){
   const r={x,y:yy-11,w,h:15,id:s.id};
   codexSummonRects.push(r);
   if(hovered(r)) line(x,yy+3,x+w,yy+3,K.gold,1);
  }
  x+=w;
 }
 return yy;
