...
const UPGRADES=[
 ...
 {id:'rico', name:'Ricochet Core', desc:'Hardened shot bounces off walls; each impact hits -4% softer', max:2, r:1, apply(p){ p.bounce+=1; p.dmgMult*=0.96; }},
 {id:'inc', name:'Incendiary Rounds', desc:'Phosphor payload sets foes burning; -3% impact damage', max:2, r:1, apply(p){ p.inc+=1; p.dmgMult*=0.97; }},
 {id:'cryo', name:'Cryo Rounds', desc:'Coolant shroud slows what it touches; pumps cost -3% rate', max:2, r:1, apply(p){ p.cryo+=1; p.fireRate*=0.97; }},
 {id:'slug', name:'Slug Rounds', desc:'+45% base dmg, bigger, -12% rate', max:2, r:1, apply(p){ p.dmgMult+=0.45; p.fireRate*=0.88; p.slug+=1; }},
 {id:'aegis', name:'Aegis Pulse', desc:'Shield emitter drinks dash power; +10% dash cooldown', max:2, r:1, apply(p){ p.aegisLvl++; p.shieldCdMax=Math.max(6,12-(p.aegisLvl-1)*3); p.shieldT=0; p.dashCdMax*=1.1; }},
 {id:'ward', name:'Warding Plate', desc:'Bolted plate turns the 1st hit every round; -5 max HP', r:0, max:1, apply(p){ p.hasWard=true; p.wardUp=true; p.maxhp=Math.max(60,p.maxhp-5); p.hp=Math.min(p.hp,p.maxhp); }},
 {id:'bulwark', name:'Bulwark Matrix', desc:'Layered plate turns 2+ hits/round; -4 max HP per layer', max:3, r:1, apply(p){ p.bulMax+=(p.bulMax?1:2); p.bulwark=p.bulMax; p.maxhp=Math.max(60,p.maxhp-4); p.hp=Math.min(p.hp,p.maxhp); }},
 {id:'mirror', name:'Crit Ward', desc:'Heavy ward turns one HEAVY blow/round; -5 max HP', max:1, r:2, apply(p){ p.hasMirror=true; p.mirrorUp=true; p.maxhp=Math.max(60,p.maxhp-5); p.hp=Math.min(p.hp,p.maxhp); }},
 {id:'barrier', name:'Ablative Barrier', desc:'Capacitor holds 50 damage once; drain -3% rate', max:1, r:2, apply(p){ p.barrier+=50; p.fireRate*=0.97; }},
 {id:'stasis', name:'Stasis Protocol', desc:'Cold cradle cheats death; -8 max HP per tier', max:3, r:2, dyn(p){ if(p.stasisTier===1) return {name:'Stasis Protocol',desc:'Stronger revival: 25% HP + charge'}; if(p.stasisTier>=2) return {name:'Stasis Protocol',desc:'Perfect revival: FULL HP + charge'}; return null; }, apply(p){ p.stasisTier=Math.min(3,p.stasisTier+1); p.stasisN=Math.min(3,p.stasisN+1); p.maxhp=Math.max(60,p.maxhp-8); p.hp=Math.min(p.hp,p.maxhp); }},
 {id:'crit', name:'Crit Matrix', desc:'Targeting lattice: +15% crit; compute steals -2% rate', max:3, r:1, apply(p){ p.critCh=Math.min(0.5,p.critCh+0.15); p.fireRate*=0.98; }},
 {id:'surge', name:'Kill Surge', desc:'Kills surge speed and rate 2.5s; hot idle -2% base rate', max:2, r:1, apply(p){ p.surgeLvl++; p.fireRate*=0.98; }},
 {id:'tract', name:'Tractor Core', desc:'Tractor mass hauls pickups (+70% magnet, +10% XP); -2% speed', max:2, r:0, apply(p){ p.magnet*=1.7; p.xpBonus*=1.1; p.speed*=0.98; }},
 {id:'magnet', name:'Magnet Core', desc:'Vacuum coils widen the pickup; power draw -2% rate', max:3, r:0, dyn(p){ return (p.magnet>90||p.pull>430)?{name:'Magnet Core',desc:'Vacuum EVERY gem + wider pickup'}:null; }, apply(p){ p.magnet*=2.2; p.pull=(p.pull||430)*1.4; p.fireRate*=0.98; collectGems(); }},
 {id:'pcell', name:'Portal Cell', desc:'ITEM: +2 recall charges (max 5)', req(p){ return p.charges<5; },
  dyn(p){ return p.recallUnlocked?{name:'Portal Cell',desc:'Spare charges ride along: +2 gate charges, -1% rate'}:{name:'Portal Cell',desc:'UNLOCK recall (E): +2 gate charges'}; }, apply(p){ if(p.recallUnlocked) p.fireRate*=0.99; p.recallUnlocked=true; p.charges=Math.min(5,p.charges+2); }},
 {id:'gatecd', name:'Gate Overdrive', desc:'Hotter gate, shorter wait; -1% rate', max:3, r:1, req(p){ return p.recallUnlocked; }, apply(p){ p.recallCdMax=Math.max(2,p.recallCdMax*0.75); p.fireRate*=0.99; }},
 {id:'transit', name:'Instant Transit', desc:'Tuned blink, shorter channel; -1% rate', max:3, r:1, req(p){ return p.recallUnlocked; }, apply(p){ p.channelMax=Math.max(0.1,p.channelMax-0.2); p.fireRate*=0.99; }},
 {id:'orbit', name:'Guardian Orbit', desc:'Each blade needs guiding; +1 orb, -2% damage', max:2, r:1, apply(p){ p.orbs+=1; p.dmgMult*=0.98; }},
 {id:'nova', name:'Frost Nova', desc:'Cryo pulse install; cycling it costs -2% rate', max:2, r:1, apply(p){ p.novaLvl++; p.novaT=Math.min(p.novaT||99,1.5); p.fireRate*=0.98; }},
 {id:'tesla', name:'Tesla Arc', desc:'Arc coils chain lightning; recharge -3% rate', max:2, r:2, apply(p){ p.teslaLvl++; p.teslaT=Math.min(p.teslaT||99,1); p.fireRate*=0.97; }},
 {id:'pierce', name:'Lance Rounds', desc:'Needle points pierce +1 foe; thinner rounds -3% damage', max:2, r:1, apply(p){ p.pierce+=1; p.dmgMult*=0.97; }},
 {id:'wind', name:'Second Wind', desc:'A backup heart beats once at 50%; -8 max HP', max:1, r:2, apply(p){ p.secondWind=true; p.maxhp=Math.max(60,p.maxhp-8); p.hp=Math.min(p.hp,p.maxhp); }},
 ...
 {id:'shock', name:'Kinetic Discharge', desc:'Kill-charge coils vent a shockwave every 50 kills; -2% rate', max:1, r:1,
  apply(p){ p.shockOn=true; p.shockNeed=50; p.shockKills=0; p.shockDmg=28; p.shockR=180; p.fireRate*=0.98; }},
 ...
 // ---------- heavy ordnance ----------
 {id:'orbital', name:'Orbital Cannon', desc:'Strike uplink calls it down; tracking -2% rate', max:3, r:1,
  dyn(p){ return p.orbitalLvl>0?{name:'Orbital Cannon',desc:'Faster cadence, +1 strike per salvo (now '+p.orbitalLvl+')'}:null; },
  apply(p){ p.orbitalLvl++; p.orbitalCd=Math.max(3.5,9-(p.orbitalLvl-1)*2); p.orbitalT=Math.min(p.orbitalT||99,3); p.fireRate*=0.98; }},
 {id:'lance', name:'Prism Lance', desc:'The prism drinks pump power; periodic beam, -2% rate', max:3, r:1,
  dyn(p){ return p.lanceLvl>0?{name:'Prism Lance',desc:'Wider beam, faster cycle (now '+p.lanceLvl+')'}:null; },
  apply(p){ p.lanceLvl++; p.lanceCd=Math.max(2.2,6-(p.lanceLvl-1)*1.4); p.lanceT=Math.min(p.lanceT||99,2); p.fireRate*=0.98; }},
 // ---------- ammunition ----------
 {id:'flak', name:'Flak Rounds', desc:'Shells burst into splash; -4% direct damage', max:2, r:1,
  apply(p){ p.flak+=1; p.dmgMult*=0.96; }},
 {id:'corrode', name:'Corrosive Rounds', desc:'Acid eats armour (+8% taken, stacks 5); volatile mix -2% rate', max:2, r:1,
  apply(p){ p.corrode+=1; p.fireRate*=0.98; }},
 {id:'chain', name:'Chain Shot', desc:'Charge arcs to a nearby foe; split load -3% damage', max:2, r:1,
  apply(p){ p.chain+=1; p.dmgMult*=0.97; }},
 {id:'overcharge', name:'Overcharge Rounds', desc:'Every 5th shot leaves a heavy slug; charge time -2% rate', max:2, r:1,
  apply(p){ p.overcharge+=1; p.fireRate*=0.98; }},
 // ---------- sustain and pressure ----------
 {id:'adrenal', name:'Adrenal Core', desc:'Damage rises as the hull falls (to +50%); taxed when calm -2% base rate', max:2, r:1,
  apply(p){ p.adrenal+=1; p.fireRate*=0.98; }},
 {id:'repair', name:'Repair Drone', desc:'Drones knit the hull after 4s calm; bay mass -2% damage', max:3, r:1,
  apply(p){ p.repair+=1.5; p.dmgMult*=0.98; }},
 {id:'shrap', name:'Shrapnel Core', desc:'The slain burst; volatile kills, smaller bullets -2% damage', max:2, r:1,
  apply(p){ p.shrap+=1; p.dmgMult*=0.98; }},
 {id:'salvage', name:'Salvage Protocol', desc:'Gems mend hull and pay +15% XP; sorting takes cycle -2% rate', max:2, r:0,
  apply(p){ p.salvage+=1; p.xpBonus*=1.15; p.fireRate*=0.98; }}
];
