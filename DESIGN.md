...
- **Draft:** three 220×204 cards at x 130 + 240i, y 220. A returning dash or recall card is a 460×62 strip below them at 250,446, not a fourth column.
 - **Codex:** one group optically centred in the game view (the canvas; the DOM key-hint line below it never centres), splitting free space 42:58 — title, two key lines, tabs (30px), an 18px break, content, a 20px break, BACK (300×44). A 200px index column (20px rows, 19px narrow), a 24px gutter, a 560×378 detail plate. The portrait sits in a 150px dashed plate inset 14px (100px on narrow windows, where both panes squeeze). The index pager owns a reserved 26px footer row and ▲ owns the gap above the first row, so ▼ MORE never sits on an entry. A faint ruled divider sits under both portrait and header; TELL / COUNTER / FIELD NOTE start below it at a ~64ch measure, so long headers (role, trait, SUMMONS links, thralls — all 11px dim, every boss identical) never touch the art or the TELL. Entry change re-engraves the plate once (rise + alpha, 220ms, off under reduced motion).
