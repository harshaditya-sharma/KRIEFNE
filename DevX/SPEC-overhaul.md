...
### Now in progress
**Step 4b done and green (`--all` 2657). No agent is running.** S50–S95 hose nests all inside 100–150 (S50 112.5, S55 118.4, S60 121.7, S65 132.6, S70 115.3, S75 110.2, S80 120.4, S85 124.7, S90 116.2, S95 116.5) and S100 at 187.8 inside 150–210, both phases. HP bases now ARCHON 11000, COLOSSUS 12000, BASILISK 8000, PROGENITOR 22000, HARBINGER 18500, KRAKEN 8000, JUGGERNAUT 6800, ECLIPSE 2000, NULLIFIER 2800, CHORUS 4500, SINGULARITY 2500. Fitted bottom-up (each nest depends on the rung below through the 45% summon chain) with a throwaway single-seed probe driving the suite's exact `simRun` seeds, then applied to `game.js` and verified through the suite. Warnings for 4c/4d: nest times couple through the chain (S100 *shortened* 196→145 when its triple got fatter — downstream RNG lottery, ±10s at S100; ECLIPSE sits above a chain-lottery cliff at ~1400, do not lower it toward the plateau). S50 balanced sits at the SIM_CAP edge (displays >400, t<400, passes — fragile, deterministic). `liveOne` keeps its 10-card build but PROGENITOR/HARBINGER run longer clocks (240s/150s, pacing pinned by fightsim; bolting hose cards onto the gun line backfires via §2 taxes). Next: 4c (recovery numbers).
...
- **[4]**
  - Boss HP: S15–S45 (4a) and S50–S100 (4b) fitted to §6, green. Open: recovery numbers (4c), S105–S130 wall checks + `FIGHTSIM_STRICT` (4d), deep danger (4e).
