...
**Where things are**
- **Branch:** `wave3` (run `git log --oneline -5`). **Merged to `main` 2026-09-16** (fast-forward, on the user's word); keep working on `wave3` and merge again only when the user says. `node test.js --all` is green (**2694**).
...
| 2 (1–5) | thralls: engine, per-kit reduced kits, director and nest chaff, draw, pigment floors, `suiteThralls` | done (e8bd147, 8154891; WIP 926c28b) |
| **2a** | make `suiteThralls` green: add "Thralls from S<n>" to `commandLine()` for eligible gods, or drop that one check | **done** (926c28b; green in the 2026-09-14 run) |
...
| 2c | one lab capture round of thralls (lesser look, pip legible, distinct from chaff) | **done 2026-09-15 (REVENANT + SENTINEL rounds, pass — see Now)** |
...
| 4e | deep danger: foe damage and behaviour, nest chaff (the user's levers) | **done (damage levers in, bands re-fit, `--all` 2679 green)** |
| 5a–5d | docs: README chain-of-command section (a); PRODUCT.md (b); LORE.md §7/§8/§11 (c); DESIGN.md pigments and banners (d) | **done (af46e4f, 46065bd, bed5227, cc5d65e; fast 2524 green)** |
| 6a–6d | full pass: lab captures of all 20 gods (a); maps and codex portraits (b); perf check (c); the user's playtest (d) | **6a done 2026-09-15 (all 20 pass, no defects); 6b + 6c done 2026-09-16 (maps PASS, codex blocker fixed, perf 0.1ms avg); 6d open** |
| 8 | lab overhaul (repo surgery + full pass for players/QA/management/agents) | **parked — low priority, after 6a–6d and pending gameplay changes (user call 2026-09-15)** |
...
### Now in progress
**Draft round done 2026-09-16 (cards read right, stats come from cards alone, the draft is a slot machine). No agent is running.** Card previews lead with one gain and one cost, short-window names fit, the Boss Bank and the +3 HP per level are gone, hull cards became live plating (`PLATE_K.div` 30) with a boss re-fit, and the draft spins three reels with a one-pull lever. `--all` **2745 green**. Next: 6d (the user's playtest), then the two capture passes. 
...
- **7b sim arc (closed).** 7b landed 7 fightsim fails (S6 72/50-70, S21 70/75-100, S31 107/75-100, S81 148/100-130 + quiet 13.3%, S99 163/100-130, S61 balanced stuck 400s) vs baseline db92f59's 3 marginals. Bisection cleared 2e (clustering innocent and directionally right); the regression was §2 taxes compounding (~25 taxed picks × ~2% ≈ ×0.7 paper DPS on wide builds).
...
- **Fight simulator with thralls (for the record; 2b closed as HOLD).**
  - First cut, before 926c28b: Hose took 138 s at S31, 164 at S46, 179 at S61, 257 at S81 and 280 at S99, against the old bands 75–100 and 100–130; the baseline without thralls was 94/85/109/111/118.
...
- **README card lines:** resolved (tree clean at the 2026-09-16 merge).

### Open items
Each is tagged with the step that owns it; resolved items are removed.
- **[2] Done 2026-09-15.** All of 2/2a–2e + 2c green and captured; step 2 closed.
- **[7, cards]** Done 2026-09-15: 7a, 7b, halved taxes, widened bands, `--all` green; blurb **kept** after verifying every claim against the card defs (one fix: lighter mounts → Mk III, Halo named; the Mk II ★ line had Mk III's tagline).
...
- **[4] Done 2026-09-15.**
  - Boss HP + recovery + wall + danger: S15–S100 fitted to §6 (4a/4b), recovery numbers tuned (4c), S105–S130 wall checks + `FIGHTSIM_STRICT` (4d), deep danger through foe damage (4e), green (`--all` 2679).
- **[5] Done 2026-09-15** (5a README, 5b PRODUCT, 5c LORE, 5d DESIGN; fast 2524 green). Open writer call: Echoes + hoards (T13–T20) for the eight new gods, tracked in LORE §13.
- **[6] Done 2026-09-16 (6b + 6c; 6d user playtest still open).**
  - Maps: 40 captures reviewed, PASS. Landed: dying-star core pushed a full radius clear of the rim (was washing under the HUD), debris 3-pylon avenue de-stamped (middle pylon ×1.12, 30° steps). No blockers.
  - Codex: 27 plates reviewed. Landed: wake discs fade with age and step to dried oxide past half-life (render-only — hitbox, life and counts untouched, so no re-fit); portrait plate 134→150px wide (registered scale stays honest; narrow stays 100); LEVIATHAN's codex curl off-centred so segments never ring the rank rings. Paperwork: header `/20`→`/26` in README + DESIGN (code counts 20 gods + 6 foes); `cx-foe-06` is a DRONE duplicate, ignore it.
  - Perf: standalone `perf` suite — the old fuzz window held the bar at 30% so Absorption (25%) never fired and Phase 2 was never measured. Now Phase 1 + Convocation and Phase 2 Quasar windows, each 120 sim-seconds, each asserted < 4 ms (measured 0.128 / 0.104 ms avg). `--all` 2689 green.
  - Still open: one zoomed capture per motif (nine-hairline star + rim ticks), unmet / met-only / boss-page-2 codex captures, and 6d.
- **[lab] Parked — low priority (user call 2026-09-15; revisit after 6a–6d and pending gameplay changes).** Full lab overhaul for non-developers (testers, alpha players, QA, management) plus agents: guided scenarios (capture rounds first), staging controls (frame target, hold fire, spawn chaff, clean capture framing), panel cleanup, DevX README refresh. Decisions already made: (1) force-track lab + dev-hooks so fresh clones have a lab (generated `game.dev.js` stays local); (2) repo surgery first — extract the ~290-line hooks block + 5 `__dev` touch-points from prod `game.js` into a dev copy (`sync-dev.mjs` generates `DevX/game.dev.js` = prod + hooks; new touch-points like follow-target/hold-fire/HUD-hide live dev-only, never ported); dev-copy-first workflow for all gameplay edits (lab-verify, then port to prod + `test.js`); (3) rewrite the 3 `test.js` checks that arm dev hooks to harness-level APIs so tests only ever boot prod.
...
| 2026-09-14 | Tax tuning (call A) | **done**: all 35 §2 taxes halved (faces + `suiteCards` regexes follow; HP floor test 65→64); §1 families untouched; fightsim 7→4 marginal fails, S61 balanced + S81 band/quiet recovered; fast 2498 green | `wave3` 6168ea0 |
...
| 2026-09-14 | §7 band re-fit + full green | **done**: targets 50–75 / 70–105 / 100–135 (`sectorBand` follows, loosening-only); fightsim 131/131; `--all` **2631 green**; `wave3` merge-ready, merge only on the user's word | `wave3` e016bba |
| 2026-09-14 | §16 handoff audit | **done**: checked every claim against the code (29 suites, codex meeting-marks-seen, bug line refs, ARCHON-has-no-radials correction); History rows added | `wave3` 0e64f21 |
...
| 2026-09-15 | Step 4e, deep danger | **done**: `EXP_DMG_LATE` 1.012→1.017 + `THRALL.dmg` 0.75→0.9 (1.018 tripped the S95/S100 farmer-bar audit); Greedy bleeds S31→S99, Hose pressured S81+, nests 1–12 bars, no two-shotting. Density/behaviour levers measured OFF (zero sector gain, nest-lottery + S61bal-cliff cost). Re-fit: COLOSSUS 10.9k, PROGENITOR 22k, JUGGERNAUT 7k, ECLIPSE 1.7k, CHORUS 4.7k, SINGULARITY 2.8k. All 20 nests in band, S100 182.2; `--all` **2679 green**. [4] closed | `wave3` 598a5c5 |
| 2026-09-15 | Step 5, docs sweep | **done**: 5a README (chain section, 20-kit roster, recovery/balance truth), 5b PRODUCT (content, terminology, wall), 5c LORE (§7 clauses, §8 twenty gods + 8 game-text entries, L04 first-summoned-god trigger), 5d DESIGN (20 pigments, dim-ring/thrall rules, banner truth). Open writer call: Echoes + hoards for the eight new gods. Fast 2524 green | `wave3` af46e4f, 46065bd, bed5227, cc5d65e |
| 2026-09-15 | BASILISK gaze 0.8×→1.2× | **done (user call)**: the petrify snapshot now hits at 1.2× with the PETRIFIED floater; combos 299, kits3 170, fightsim 153 green | `wave3` |
| 2026-09-15 | Blurb call | **done (kept, one fix)**: every claim verified vs card defs; Mk II ★ line had Mk III's tagline — lighter mounts → Mk III, Halo named | `wave3` |
| 2026-09-15 | Step 2c, thrall captures | **done**: REVENANT + SENTINEL lab rounds pass (lesser look, pip + diamond legible, distinct from chaff). Step 2 closed | `wave3` |
| 2026-09-15 | Step 6a, 20-god captures | **done**: all 20 leads captured frozen-idle at debuts, all pass — whole distinct silhouettes, correct pigments, rings by tier, legible labels; PROGENITOR full hull confirmed | `wave3` |
| 2026-09-15 | LEVIATHAN body check | **not a bug**: live whip→coil→lunge renders head + 5 segments attached; frozen head-only is the burrow rest state | `wave3` |
| 2026-09-15 | Wake overlap gate (user call) | **done**: `dropDisc` refuses a disc overlapping a live one >50% (0.8·min-radii on current radii; shrink kept); rule pinned by 4 prims unit checks, kits asserts re-fit to coverage (wyvern even spacing, levi path span, kraken sweep span). Side-effect: S25 nest slowed (unsuppressed REVENANT hazards) → LEVIATHAN 5500→4000, S25 85.5 / S30 78.9; `--all` **2687 green** | `wave3` |
| 2026-09-16 | Step 6b, maps + portraits | **done**: 40 map captures PASS (star core past rim, trio de-stamped); 27 codex plates reviewed, blocker fixed render-only (wake age-fade + dried oxide; plate 134→150px honest-scale; LEVIATHAN curl off-centre; header `/26` in README/DESIGN) | `wave3` fa11841 |
| 2026-09-16 | Step 6c, perf floor | **done**: standalone `perf` suite split out of fuzz — Phase-1 window kept, Phase-2 window added (old 30% clamp never fired Absorption); Phase 1 0.128 / Phase 2 0.104 ms avg vs the 4 ms floor; README slow-suite list follows; `--all` **2689 green** | `wave3` b932bf8 |
| 2026-09-16 | Codex layout + lore rework (user call) | **done**: index + plate one centred group (200 + 24 + 560, ~64ch measure, locked hint centred); index reversed ENFORCER-up, S5 first; header rewritten as lore — no `Answers to`, no debut sector (the index already says it), SUMMONS line with click-through links in each god's pigment (unmet stay `???` and inert), link jump pinned by 4 codex checks; README/DESIGN follow; `--all` **2693 green** | `wave3` |
| 2026-09-16 | Codex overdrive (user call) | **done**: index pager owns a reserved footer row (rows never slide under MORE); ruled divider under portrait + header so TELL clears long headers and wide sprites; whole stack optically centred in the game view (bottom key hints excluded), BACK follows the plate; one authored motion only (plate re-engrave on entry/page/tab change, 220ms, REDUCED-gated); OVERLORD counter `calls a pack`→`summons chaff` (pinned); `--all` **2694 green** | `wave3` |
...
| 2026-09-16 | merge to main | **done**: `--all` **2694 green** on `wave3`; `main` fast-forwarded to `wave3` (6a–6c, codex rework + overdrive, cards 7a/7b, steps 2–5); both branches pushed. 6d (user playtest) still open | `main` |
| 2026-09-16 | stale-build cache fix | **done**: `vercel.json` served `game.js`/`styles.css` as `max-age=31536000, immutable` under fixed names, so returning players never fetched new builds. Now `max-age=0, must-revalidate` (ETag 304 when unchanged); `index.html` loads `?v=2` once to break the old year-long browser caches — never bump it again. `srmirror` asserts both files revalidate (+2); fast 2539 green | `wave3` |
| 2026-09-16 | draft card previews (user report: Overclock Star showed only DMG ×1.08 → ×1.03) | **done**: the math was right (−5% bleed on the hull's ×1.08); the card clipped to one line in `STAT_VIEW` order, so ~30 taxed cards showed only their red cost and 14 cards with 3-line text showed nothing. One `statRows` (gains first, then costs), a finer read when a tax rounds to "4.5 → 4.5", `FLAG_VIEW` OFF → ON rows for ward/crit ward/second wind/discharge. Desktop row card 204 → up to 228 (only as far as the BUILD gap allows); short and stacked cards draw gain + cost. Captures at 960×640, 1024×560, 420×860 clean. Tests: every card shows a gain first, no flat lines, card ≥ 228 at H ≥ 544 (+16); `--all` **2712 green** | `wave3` |
| 2026-09-16 | Step B, no stat outside a card (user rule) | **done**: Boss Bank removed (`newPlayer(1)`; lifetime kills stay a record, copy reworded in intro/help/death/pause/hub/draft/SR + PRODUCT/LORE/README) and the +3 max HP per level dropped. Hull cards became **live plating**: `plateU` units worth `plateK()=1+sector/PLATE_K.div` (div 30), re-fitted on every `loadArena` — a draft-time scale was useless because the 12-pick family cap lands early. Re-fit: REVENANT 2900→3400, PROGENITOR 22000→26500, NULLIFIER 2800→4200, LEVIATHAN 4000→3700, SINGULARITY 2800→2500 (its chain carries S100 now). Harness: `draftBuild(..., upTo)` drafts down the trail, the farmer bar reads on arrival, and the sim keeps its own guns-first balanced pilot (`SIM_BALANCED`) while `BALANCED_ORDER` stays the armoured farmer. Farmer bar S95 0.30 (was 0.93 with the passive gone), fightsim **153/153**, `--all` **2726 green** | `wave3` |
| 2026-09-16 | Step C, slot-machine draft + lever (user call) | **done**: `rollPicks` is the one sampler for the draft and the reroll; `spinReels`/`landReels` turn three reels that land left to right (260/160ms, `SPIN.tick` 70ms faces, skipped under `REDUCED`), and a press while they turn is a stop, never a pick (codex/help/lever still answer). `pullLever` re-spins the three cards once per draft (`draftPulls`), keeps the offered-again card and never arms on the starter draft. Drawn as a post-and-ball beside the row, a PULL [R] button when stacked; [R] key, click rect, hint line, help line and the SR summary all carry it. New `slot` suite (19). `--all` **2745 green** | `wave3` |
| 2026-09-16 | Draft as a refit cylinder (impeccable animate) | **done**: the draft reads as a machine — chassis rails, detent jambs, gold seat marks, and three reels that roll a 9-face strip past each aperture and seat left to right (520ms +170/reel), overshooting the stop by 0.1 face and being pulled back onto it. Speed is line density (a hairline comb with the engraving repeating down it at falling opacity), never blur. The lever is cast: mount plate, bolts, pivot, ratchet track, shaft and ball; it throws in 150ms, springs back in 430ms, stays solid through the swing and goes dashed/hollow spent. Audio follows the mechanism (`SFX.lever`/`ratchet`/`thunk(i)`, rare chime on a rare seat). Reel faces use their own dice (`reelRnd`) — the seeded stream was shifting every drafted build, caught by the farmer-bar audit. `slot` suite 32, `--all` **2758 green**, design detector clean, DESIGN.md records the cylinder | `wave3` |
| 2026-09-16 | cha-ching + PHANTOM hull (user) | **done**: `SFX.chaching` (two struck bells a fifth apart) lands when the last reel seats, when an early stop loads the hand, and at open under reduced motion. PHANTOM 360→410 hp (+14%) on the user report that it is obliterated too easily; S15 77.6s stays inside the 60–80 band (395 measured 83.4 — the lever is chaotic near the ceiling), all 20 nests in band, S61 balanced 357s. `--all` **2758 green** | `wave3` |
| 2026-09-16 | teleport telegraphs (user: PHANTOM blinks are jarring) | **done**: every legal jump is now armed and marked first — `blinkArm` shows a dashed red arrival reticle with closing cross ticks and the ruled path, `blinkTick` crosses when the mark runs out (bossUpdate + thrallUpdate), and a `then` callback keeps each kit sequencing off the arrival. Leads: PHANTOM 0.3s, CHORUS 0.4s, ECLIPSE/NULLIFIER 0.5s. All four teleporters converted (PHANTOM blinkfan/afterimage/crossfire, ECLIPSE Totality Step, NULLIFIER Silent Step — the jam now lands on arrival, CHORUS echo swap) plus the PHANTOM thrall. `SFX.warp` rises into the crossing. Re-fit forced by the added beats: NULLIFIER 4200→3100, SINGULARITY 2500→2050, CHORUS 4700→4400 (PHANTOM held at 410 — S15 is a one-seed lottery: 410→61.9s, 415→57.7, 430→82.0). New teleport check: every blink is marked ≥0.2s and lands within 8px of its mark. All 20 nests in band, `--all` **2763 green** | `wave3` |
| 2026-09-16 | PHANTOM 500 + early band ceiling (user playtest) | **done**: user report — with a stacked build PHANTOM died in ~12–13s, before the WARDEN it called had finished spawning. 410→**500** hp (+39% on the 360 baseline). The sim reads S15 at 95.4s, so the §6 early ceiling went 80→100: the scripted Hose pilot chases a blinker far worse than a person, so its clear time bounds the sim, not the felt fight (the user is the authority on felt difficulty). S20 68.1s, every nest in band, `--all` **2763 green**. Open offer: armour / burst response if 500 still folds | `wave3` |
