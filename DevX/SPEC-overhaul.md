...
### Now in progress
**6b + 6c + codex rework + codex overdrive done 2026-09-16; `--all` 2694 green; `wave3` fast-forwarded into `main` and both pushed (user call 2026-09-16). No agent is running.** 6b: 40 map captures PASS (no blockers; star core + trio fixes landed), 27 codex plates reviewed — the one blocker (Coil wake reads as dense red) fixed render-only, plate 150px, LEVIATHAN curl off-centred. 6c: standalone `perf` suite — Phase 1 0.128 ms avg, Phase 2 0.104 ms avg, both far under the 4 ms floor. `--all` **2689 green**. Next: 6d (the user's playtest). The deep-danger paragraph below stays the record for 4e. `EXP_DMG_LATE` 1.012→1.017 (1.018 tripped the farmer-bar audit at S95/S100, shaved to 1.017) and `THRALL.dmg` 0.75→0.9. The danger gradient is real: Greedy bleeds from S31 (1.4 bars) through S99 (2.7–3.7 bars, 1–2 downs); Hose takes 1–3 bars S81+ and 2–3 bars + downs at S99; nests run 1–12 bars with regular downs. No two-shotting (farmer-bar audit green). All 20 nests in band (S15 64.5, S20 62.1; S25–S45 78–90; S50–S95 106–130; S100 182.2), wall still lands at S125, `FIGHTSIM_STRICT` holds. Re-fit forced by the levers: COLOSSUS 13k→10.9k, PROGENITOR 19k→22k, JUGGERNAUT 6.8k→7k, ECLIPSE 2k→1.7k (2000 is a chain-lottery peak under the new dynamics), CHORUS 4.5k→4.7k, SINGULARITY 2.5k→2.8k. **Measured call (density/behaviour levers OFF):** `cap` [45,70], `huntR` 120 and `nestP` 0.45 add ~zero sector danger over damage-only (S99 Hose 2.6/2.8 vs 2.1/3.6, Greedy 5.0/4.0 vs 5.7/6.3) while flipping nest lotteries (5–6 nests out of band) and capping S61-balanced unfixably (15+ downs, no single trim saves it — combination cliff). Hose S21–S61 still takes ~0 (range overkill, structural). Reversing the call is one line + a nest re-fit.
